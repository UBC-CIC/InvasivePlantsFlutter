import 'dart:convert';
import 'dart:io';
import 'package:flutter_app/GlobalVariables.dart';
...
Future<Map<String, Object>> webscrapeWikipedia(String scicName) async {
  ...
  try {
    ...
    var response = await http.get(Uri.https(
        mediaWikiApiEndpoint_baseURL, mediaWikiApiEndpoint_path, params));
    ...
    for (int i = 0; i < sections.length; i++) {
      ...
      if (!sectionTitle.contains("References")) {
        String sectionValue =
            await fetchSectionContent(pageId, sections[i]["index"]);
        sectionData.add({"header": sections[i]["line"], "body": sectionValue});
      }
    }
    ...
    if (page['images'] != null && page['images'] is List) {
      imageInfo = await fetchImageUrls(
          page['images'], scientificName, redirectedPageTitleName);
    } else {
      imageInfo = [];
    }
    ...
    var wikiInfo = {
      ...
      'wikiUrl':
          'https://en.wikipedia.org/wiki/${Uri.encodeComponent(scientificName)}',
    };
...
Future<List<dynamic>> fetchSections(String pageId) async {
  ...
  var uri = Uri.https(
      mediaWikiApiEndpoint_baseURL, mediaWikiApiEndpoint_path, sectionsParams);
...
Future<String> fetchSectionContent(String pageId, String sectionIndex) async {
  ...
  // Make HTTP request
  var uri = Uri.https(
      mediaWikiApiEndpoint_baseURL, mediaWikiApiEndpoint_path, sectionParams);
  ...
  if (sectionResponse.statusCode == 200) {
    ...
    document.querySelectorAll('p').forEach((Element pElement) {
      ...
      if (pElement
          .querySelectorAll(
              'style[data-mw-deduplicate="TemplateStyles:r1154941027"]')
          .isEmpty) {
...
Future<List<String>> fetchImageUrls(
    List images, String prevTitle, String pageTitle) async {
  List<String> imageUrls = [];
  var matchNameRedirectedTitle =
      pageTitle.split(' ').map(Uri.encodeComponent).toList();
  var matchNameOriginalTitle =
      prevTitle.split(' ').map(Uri.encodeComponent).toList();
  int numImage = 0;
  ...
  for (var image in images) {
    ...
    var uri = Uri.https(
        mediaWikiApiEndpoint_baseURL, mediaWikiApiEndpoint_path, imageParams);
    var response = await http.get(uri);
    ...
    if (page != null &&
        ...
        (matchNameRedirectedTitle.any((name) =>
                Uri.decodeComponent(page['imageinfo'][0]['url'])
                    .toLowerCase()
                    .contains(name)) ||
            matchNameOriginalTitle.any((name) =>
                Uri.decodeComponent(page['imageinfo'][0]['url'])
                    .toLowerCase()
                    .contains(name)))) {
